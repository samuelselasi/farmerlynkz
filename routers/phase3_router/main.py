...
async def create_end_of_year_review(payload: schemas.create_end_of_year_review, db: Session = Depends(get_db)):
    return await crud.create_end_of_year_review(payload, db)



# @router.post("/organizationandmanagement/")
# async def create_core_competency_organization_and_management(payload: schemas.create_annual_appraisal, db: Session = Depends(get_db)):
#     return await crud.create_cc_organization_and_management(payload.assessment, payload.score, payload.weight, payload.comment, payload.appraisal_form_id, db)

# @router.post("/innovationandstrategicthinking/")
# async def create_core_competency_innovation_and_strategic_thinking(payload: schemas.create_annual_appraisal, db: Session = Depends(get_db)):
#     return await crud.create_cc_innovation_and_strategic_thinking(payload.assessment, payload.score, payload.weight, payload.comment, payload.appraisal_form_id, db)

# @router.post("/leadershipanddecisionmaking/")
# async def create_core_competency_leadership_and_decision_making(payload: schemas.create_annual_appraisal, db: Session = Depends(get_db)):
#     return await crud.create_cc_leadership_and_decision_making(payload.assessment, payload.score, payload.weight, payload.comment, payload.appraisal_form_id, db)

# @router.post("/organizationandmanagement/")
# async def create_core_competency_organization_and_management(payload: schemas.create_annual_appraisal, db: Session = Depends(get_db)):
#     return await crud.create_end_of_year_review(payload.assessment, payload.score, payload.weight, payload.comment, payload.appraisal_form_id, db)
