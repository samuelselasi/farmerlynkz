...
async def approve_form_details_mid(appraisal_form_id:int, type_form='Mid', token:str=Depends(oauth2_scheme), db:Session=Depends(get_db)):
    return await crud.approve_form_details_mid_auth(appraisal_form_id, type_form, token, db)
