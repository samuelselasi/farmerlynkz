...
from main import settings, scheduler
...
async def request_password_reset(payload:schemas.UserBase, db:Session, background_tasks):
    try:
        ...
        if not user:
            raise NotFoundError('user not found')
        while True:
            new_code = models.ResetPasswordCodes(user_id=user.id, code=models.ResetPasswordCodes.generate_code()) # GENERATE CODE TO RESET PASSWORD
...
async def request_password_reset_(payload:schemas.UserBase, db:Session, background_tasks):
    try:
        ...
        if not user:
            raise NotFoundError('user not found')
        while True:
            new_code = models.ResetPasswordCodes(user_email=user.email, code=models.ResetPasswordCodes.generate_code()) # GENERATE CODE TO RESET PASSWORD
            code = db.query(models.ResetPasswordCodes).filter(models.ResetPasswordCodes.user_email==user.email).first()
            if code:
                db.delete(code) # DELETE OLD CODE FROM DB
                db.flush()
            break
...
async def get_current_user(token:str, db:Session):
    try:
        ...
        if token_data:
            return await read_user_by_id(token_data['id'], db) # RETURN  DETAILS OF LOGGED IN USER
    except UnAuthorised:
