...
async def create_end_of_year_review(payload: schemas.create_mid_year_review, db: Session = Depends(get_db)):
    return await crud.create_end_of_year_review(payload.progress_review, payload.remarks, payload.status, payload.appraisal_form_id, payload.annual_plan_id, payload.staff_id, db)

@router.post("/corecompetencies/")
async def core_competencies(payload: schemas.appraisal_form, db: Session = Depends(get_db)):
    return await crud.core_competencies(payload.department, payload.grade, payload.positions, payload.date, payload.staff_id, db)
...
@router.delete("/endofyearreview/{end_of_year_review_id}/")
async def delete_end_of_year_review(end_of_year_review_id: int, db: Session = Depends(get_db)):
    return await crud.delete_end_of_year_review(end_of_year_review_id, db)

@router.delete("/corecompetencies/{core_comepetencies_id}/")
async def delete_core_competencies(core_competencies_id: int, db: Session = Depends(get_db)):
    return await crud.delete_core_competencies(core_competencies_id, db)
